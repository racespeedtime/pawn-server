...
CMD:animhelp(playerid, params[]) {
    ...
    new msg[128];
    format(msg, sizeof msg, " | - /animsave <comment> {FFFF00}Save the animation to '%s' with a comment.", ANIM_SAVE_FILE); 
    SendClientMessage(playerid, -1, msg);
    // SendClientMessage(playerid, -1, " | - /animsave <comment> {FFFF00}Save the animation to '"
    //     #ANIM_SAVE_FILE "' with a comment.");
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[]) {
    ...
    if (dialogid == D_ANIM_IDX && response) {
        ...
        if (0 < idx < MAX_ANIMS) {
            ...
            new msg[128];
            format(msg, sizeof msg, "Enter an animation index number (idx)\nbetween 0 and %d", MAX_ANIMS); 
            ShowPlayerDialog(playerid, D_ANIM_IDX, DIALOG_STYLE_INPUT, "Anim Index", msg, "Enter", "Cancel");
            // ShowPlayerDialog(playerid, D_ANIM_IDX, DIALOG_STYLE_INPUT, "Anim Index", "Enter an animation index number (idx)\nbetween 0 and "
            //     #MAX_ANIMS ":", "Enter", "Cancel");
        }
    }
    return 1;
}
