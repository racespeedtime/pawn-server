// 适配流光 By YuCarl77 版权(c) <2019-2020> <YuCarl77>([R_ST]Hygen)
// -----------------------------------------------------------------------------
// -----------------------------------------------------------------------------
...
public OnFilterScriptInit()
{
    ...
    LSApartments1Object = CreateDynamicObject(19595, 1160.96, -1180.58, 70.4141, 0, 0, 0);

    // Display information in the Server Console
    print("  |--  LS Apartments 1 Building object created");
    
    // Create the LS Apartments 1 Car Park object
    LSApartments1CPObject = CreateDynamicObject(19798, 1160.96, -1180.58, 20.4141, 0, 0, 0);
...
public OnFilterScriptExit()
{
	...
	if (IsValidDynamicObject(LSApartments1Object))
	{
		// Destroy the LS Apartments 1 Building object
		DestroyDynamicObject(LSApartments1Object);
    ...
    // Check for valid object
	if (IsValidDynamicObject(LSApartments1CPObject))
	{
		// Destroy the LS Apartments 1 Car Park object
		DestroyDynamicObject(LSApartments1CPObject);
...
public OnDynamicObjectMoved(objectid)
{
	...
	if(objectid == Obj_Elevator)   // The elevator reached the specified floor.
	{
	    ...
	    Label_Elevator	= CreateDynamic3DTextLabel("{CCCCCC}Press '{FFFFFF}~k~~CONVERSATION_YES~{CCCCCC}' to use elevator", 0xCCCCCCAA, X_ELEVATOR_POS - 1.7, Y_ELEVATOR_POS - 1.75, z - 0.4, 4.0);
...
stock Elevator_Initialize()
{
	...
	Label_Elevator = CreateDynamic3DTextLabel("{CCCCCC}Press '{FFFFFF}~k~~CONVERSATION_YES~{CCCCCC}' to use elevator", 0xCCCCCCAA, X_ELEVATOR_POS - 1.7, Y_ELEVATOR_POS - 1.75, GROUND_Z_COORD - 0.4, 4.0);
	...
	for (new i; i < sizeof(Obj_FloorDoors); i ++)
	{
		...
		// Create floor label
		Label_Floors[i] = CreateDynamic3DTextLabel(string, 0xCCCCCCAA, X_ELEVATOR_POS - 2.5, Y_ELEVATOR_POS - 2.5, z - 0.2, 10.5);
	}
