...
public OnDynamicObjectMoved(objectid) {
    ...
    if(objectid == Obj_Elevator) // The elevator reached the specified floor.
    {
        ...
        Label_Elevator = CreateDynamic3DTextLabel("{CCCCCC}Press '{FFFFFF}~k~~CONVERSATION_YES~{CCCCCC}' to use elevator", 0xCCCCCCAA, X_ELEVATOR_POS + 1.6, Y_ELEVATOR_POS - 1.85, z - 0.4, 4.0);
...
stock Elevator_Initialize() {
    ...
    Label_Elevator = CreateDynamic3DTextLabel("{CCCCCC}Press '{FFFFFF}~k~~CONVERSATION_YES~{CCCCCC}' to use elevator", 0xCCCCCCAA, X_ELEVATOR_POS + 1.6, Y_ELEVATOR_POS - 1.85, GROUND_Z_COORD - 0.4, 4.0);
    ...
    for (new i; i < sizeof(Obj_FloorDoors); i++) {
        ...
        // Create floor label
        Label_Floors[i] = CreateDynamic3DTextLabel(string, 0xCCCCCCAA, X_ELEVATOR_POS + 2, Y_ELEVATOR_POS - 3, z - 0.2, 10.5);
    }
...
stock ShowElevatorDialog(playerid) {
    ...
    ShowPlayerDialog(playerid, LS_BeachSideID, DIALOG_STYLE_LIST, "LS BeachSide Elevator...", string, "Accept", "Cancel");
